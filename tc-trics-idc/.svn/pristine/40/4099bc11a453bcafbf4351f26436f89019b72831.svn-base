...
public class DeviceServiceImpl implements DeviceService {
    ...
    public void addDevice(Res res) {
        ...
        res.setAssetId(sysOrg.getFullCode()+"-"+res.getResTypeCode()+"-"+RandomUtile.getRandomString(8));
    ...
    public void delDevice(Long id) {
        ...
        Res res = new Res();
        res.setId(id);
        Res res1 = resMapper.selectOne(res); //查询此设备是否有标签
        if(res1.getTid()!=null){
            Label label = new Label();
            label.setTid(res1.getTid());
    ...
    public void exportDeviceModel(HttpServletResponse response) throws IOException {
        ...
        ResExtCfg query = new ResExtCfg();
        query.setResTypeCode("DEV");*/
        List<ResExtCfg> dev =  resExtCfgMapper.selectByResTypeCode("DEV");
           ...
           if(null != resType){
            ...
            for (String key:map.keySet()) {
                ...
                if(StringUtils.isNotEmpty(value)){
                    ResExtCfg resExtCfg = new ResExtCfg();
                    resExtCfg.setItemCode(key);
                    resExtCfg.setIsDel(IdcConstant.SYSTEM_ISDEL.NOT_DEL);
                    ResExtCfg query = resExtCfgMapper.selectOne(resExtCfg);

                    String valType = query.getValType();

                    SysParam sysParam = new SysParam();
                    sysParam.setId(Long.parseLong(valType));
                    sysParam.setIsDel(IdcConstant.SYSTEM_ISDEL.NOT_DEL);
                    SysParam queryPram = sysParamMapper.selectOne(sysParam);
                    String sysKey = queryPram.getSysKey();
                    ...
                    switch (sysKey){
                        case "date":
                            value = StringUtile.dateFormat(value);
                            boolean dateTime = StringUtile.isDateTime(value);
                            if(dateTime){
                                String ps = maps.get("ps").toString();
                                ps += "["+query.getItemName()+"不符合日期规范(格式YYYY-MM-DD)]";
                                maps.put("ps",ps);
                                back.add(maps);
                                extFlag = false;
                            }
    ...
    public void untyingLabel(Res res) {
        String isPasted = res.getIsPasted();
        res = resMapper.selectOne(res);
        String tid = res.getTid();
        res.setUpdatedTime(new Date());
        res.setTid("");
        res.setIsPasted(IdcConstant.SYSTEM_PASTED.NOT_PASTED+"");
        res.setIsPrinted(IdcConstant.SYSTEM_PRINT.NOT_PRINT);
        ...
        Label label = new Label();
        label.setTid(tid);
        if("1".equals(isPasted)){
            label.setIsDel(IdcConstant.SYSTEM_ISDEL.DEL);
            label.setIsBinding(IdcConstant.SYSTEM_ISBIND.BIND);
            label.setIsPasted(IdcConstant.SYSTEM_PASTED.PASTED);
            label.setIsPrinted(IdcConstant.SYSTEM_PRINT.PRINT);
        }else{
            label.setIsDel(IdcConstant.SYSTEM_ISDEL.NOT_DEL);
            label.setIsBinding(IdcConstant.SYSTEM_ISBIND.NOT_BIND);
            label.setIsPasted(IdcConstant.SYSTEM_PASTED.NOT_PASTED);
            label.setIsPrinted(IdcConstant.SYSTEM_PRINT.NOT_PRINT);
        }
        label.setUpdatedTime(new Date());
        labelMapper.updateByTid(label);
    }
