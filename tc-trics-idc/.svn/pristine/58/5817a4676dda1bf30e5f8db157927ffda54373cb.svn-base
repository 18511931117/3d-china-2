...
public class RepairTaskController {
    ...
    public ResultResponse desc(RepairTask repairTask,PageRequest pageRequest) {
        PageInfo<RepairTaskDesc> pageInfo =repairTaskService.desc(repairTask,pageRequest);
    ...
    @RequestMapping(value = "/app/updateAppRepairTaskState", method = RequestMethod.POST)
    @ApiOperation(value = "维修任务状态变更", httpMethod = "POST")
    public ResultResponse updateAppRepairTaskState(RepairTask repairTask) {
        repairTaskService.updateState(repairTask);
        return DataSetUtile.setSuccessResult(null);
    }
    //***********************************手持机维修任务接口**************************************//
