...
//     table.render({
//         elem: '#test'
//         ,url:ctxPath+'repairTask/getPage'
//         ,title: '用户数据表'
//         ,cols: [[
//             {type: 'checkbox'},
//             {field: 'id', title: 'ID',}
...
//                     if(data.code=="0"){
//                         setTimeout("javascript:location.href='/repairTask'", 1500);
//                     }
//                 },
//                 error:function(err){
//                     layer.msg("服务器内部错误！")
...
//             console.log(data.id)
//             window.location.href = "repairTask/toDesc?id="+data.id
...
$('input').each(function () {
    ...
    function updateState(id,state){
            $.ajax({
                url:  ctxPath+"repairTask/updateState",
                type: "POST",
                data:{
                    id:id,
                    state: state
                ...
                success: function(data){
                    ...
                    if(data.code=="0"){
                        setTimeout(window.location.href=ctxPath+'repairTask', 1500);
                    }
                },
                error:function(err){
                    var obj={
...
window.operateEvents = {
    ...
    'click #bind': function (e, value, row, index) {
        window.location.href = ctxPath+"repairTask/toDesc?id="+row.id
    },
    // 盘点任务撤销
...
function addFunctionAlty(value, row, index) {
    ...
    $("#operateDiv").find("button").each(function(){
        ...
        if(row.orgId!=orgId || (row.orgId == orgId && row.orgPid == '0')){
