...
public class CableController {
    ...
    @RequiresPermissions("cable:importCable")
    @ResponseBody
    ...
    public ResultResponse importCable(MultipartFile cableFile){
        List<CableExt> b = null;
        try {
            b = cableService.importCable(cableFile);
            if(b == null){
    ...
    @RequiresPermissions("cable:addCable")
    @ResponseBody
    @ApiOperation(value = "导入线缆", httpMethod = "POST", response = ResultResponse.class)
    ...
    @RequiresPermissions("cable:exportCable")
    ...
    @RequiresPermissions("cable:print")
    ...
    @ResponseBody
    @Operation(operation = "批量确认贴标")
    @ApiOperation(value = "批量确认贴标", httpMethod = "POST", response = ResultResponse.class)
    public ResultResponse batchConfirm(@RequestBody List<Long> idList){
        Cable cable = null;
        for (int i = 0; i <idList.size() ; i++) {
            cable = new Cable();
            cable.setId(idList.get(i));
            cableService.confirm(cable);
        }
        return DataSetUtile.setSuccessResult(null);
    }
    ...
    @ResponseBody
    @RequiresPermissions("cable:untyingLabel")
    ...
    @ResponseBody
    @RequiresPermissions("cable:batchPrint")
    ...
    @RequiresPermissions("cable:bindPcExport")
    @ResponseBody
    ...
    @RequiresPermissions("cable:bindPhoneExport")
    @ResponseBody
    @ApiOperation(value = "手持机贴标导出", httpMethod = "POST", response = ResultResponse.class)
    public ResultResponse bindPhoneExport(@RequestParam(name="ids",required = false) List<Long> ids,HttpServletResponse response){
        cableService.bindPhoneExport(ids,response);
        return DataSetUtile.setSuccessResult(null);
    }
}
