...
public class CableServiceImpl implements CableService {
    ...
    @Override
    @Transactional
    public List<CableExt> importCable(MultipartFile cableFile) throws IOException {

        File file = FileToUtile.multipartFileToFile(cableFile);

        String[] title={"cableType","intf1Type","room1Name","res1Code","port1Id","intf2Type","room2Name","res2Code","port2Id","serveInfo"};
        ...
        List<CableExt> back = new ArrayList<>();
        for (CableExt ext:
             list) {
    ...
    public String appBindLabel(Cable cable) {
        ...
        if(!StringUtils.isNotEmpty(tid) || !StringUtils.isNotEmpty(tid2)){
            return "源端宿端标签均不可为空!";
        }
        Cable query = new Cable();
        query.setId(cable.getId());
        query = cableMapper.selectOne(query);
        if(StringUtils.isNotEmpty(query.getTid()) || StringUtils.isNotEmpty(query.getTid2())){
            return "该线缆已绑定标签!";
        }
    ...
    public PageInfo<CableExt> getAppCablePage(CableQuery cableQuery, PageRequest pageRequest) {
        ...
        List<CableExt> list = cableMapper.selectByCondition(DataSetUtile.setQueryMap(cableQuery,pageRequest,SessionUtil.getUser()));
    ...
    public void untyingLabel(Cable cable) {
        ...
        Integer isPasted = cable.getIsPasted();
