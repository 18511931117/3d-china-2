...
public class RackController {
    ...
    public ModelAndView toAddRack(ModelAndView model){
        model.setViewName("system/rackAdd");
        return model;
    }


    @GetMapping("toEditRack/{id}")
    @ApiOperation(value = "添加机架页面", httpMethod = "GET", response = ResultResponse.class)
    public ModelAndView toEditRack(ModelAndView model,@PathVariable("id") Long id){
        Rack rack = new Rack();
        rack.setId(id);
        rack = rackService.getPage(rack, null).getList().get(0);
        model.setViewName("system/rackEdit");
        model.addObject("bean",rack);
        return model;
    }
