...
//            更新设备状态为盘点任务中
    ...
    public void updateState(AllocationTask allocationTask) {
        ...
        if(allocationTask.getState()==TaskState.SUCCESS_TASK.getState()){
            ...
            //根据orgId查询区域ID
            Long endOrgId = allocation.getEndOrgId();
            SysOrg sysOrg = new SysOrg();
            sysOrg.setId(endOrgId);
            SysOrg sysOrg1 = sysOrgMapper.selectOne(sysOrg);
            byTaskIdResIdList.stream().forEach(resId->super.updateResStatus(resId,"1",allocation,Long.parseLong(sysOrg1.getRegionId())));
        }
