...
//            byTaskIdResIdList.stream().forEach(resId-> super.updateResStatus(resId,"6"));
        }
    }
    ...
    public List<ScrapTaskDesc> export(ScrapTask scrapTask) {
        ...
        Arrays.stream(scrapTask.getIds().split(",")).forEach(id->
                ids.add(Long.parseLong(id)));
        return  scrapTaskDescMapper.selectByExport(ids);
    }
