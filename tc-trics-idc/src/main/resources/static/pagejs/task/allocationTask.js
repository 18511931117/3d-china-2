...
//     table.render({
//         elem: '#test'
//         ,url:ctxPath+'allocationTask/getPage'
//         ,title: '用户数据表'
//         ,cols: [[
//             {type: 'checkbox'},
//             {field: 'id', title: 'ID',}
...
//             //添加任务
//             $.ajax({
//                 url: ctxPath+"allocationTask/add",
//                 type: "POST",
//                 data:{
//                     taskName:taskName,
//                     remark:remark,
...
//                     if(data.code=="0"){
//                         setTimeout("javascript:location.href=ctxPath+'allocationTask'", 1500);
//                     }
//                 },
//                 error:function(err){
//                     layer.msg("服务器内部错误！")
...
//                 $.ajax({
//                     url:  ctxPath+"allocationTask/delete",
//                     type: "POST",
//                     data:{
//                         id:data.id
//                     },
...
//             console.log(data.id)
//             window.location.href = ctxPath+"allocationTask/toDesc?id="+data.id
...
//     //更新任务状态
    function updateState(id,state){
            $.ajax({
                url:  ctxPath+"allocationTask/updateState",
                type: "POST",
                data:{
                    id:id,
                    state: state
                ...
                success: function(data){
                  // alert(data.message);
                    ...
                    if(data.code=="0"){
                        setTimeout(window.location.href=ctxPath+'allocationTask', 1500);
                    }
                },
                error:function(err){
                  // alert("服务器内部错误！")
...
window.operateEvents = {
    ...
    'click #bind': function (e, value, row, index) {
        window.location.href = ctxPath+"allocationTask/toDesc?id="+row.id
    },
    // 调拨任务撤销
...
function initTable() {
    $('#test3').bootstrapTable({
        url: ctxPath + 'device/getDevicePage?roomId='+$("#startRoom").val(),
        method: 'GET',
        uniqueId: 'id',                        // 绑定ID，不显示
        striped: false,                         //是否显示行间隔色
        cache: false,                          //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
...
$('#myModal2').on('hidden.bs.modal', function () {
    ...
    if (data.length > 0) {
        ...
        //
        // $("#test2").bootstrapTable("load", allTableData);
